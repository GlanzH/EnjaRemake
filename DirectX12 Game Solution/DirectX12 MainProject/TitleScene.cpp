...
{
	curtain_pos = SimpleMath::Vector3(0,0,1.0f);
...
void TitleScene::LoadAssets()
{
	...
	titlelogo = DX9::Sprite::CreateFromFile(DXTK->Device9, L"Scene/Title/TitleLogo.png");

	noise = DX9::MediaRenderer::CreateFromFile(DXTK->Device9, L"BGM_SE/Title/before_the_performance.mp3");
	noise->Play();
...
void TitleScene::Render()
{
    ...
    DX9::SpriteBatch->DrawSimple(titlelogo.Get(),SimpleMath::Vector3(0.0f, -100.0f, 0.0f),nullptr,
        DX9::Colors::RGBA(255, 255, 255, title_logo_alpha));

    DX9::SpriteBatch->DrawSimple( prologue_text.Get(),SimpleMath::Vector3(0.0f, 0.0f, 4.0f),nullptr,
        DX9::Colors::RGBA(255, 255, 255, protext_alpha));

    if (pv_play_flag) {
        DX9::SpriteBatch->DrawSimple(pv->Get(),SimpleMath::Vector3(0.0f, 0.0f, -20.0f));
    }
...
cppcoro::generator<int> TitleScene::Opening() {
    ...
    while (true) {
        ...
        if (!game_start_flag)
            protext_alpha = std::min(protext_alpha + 100.0f * time_delta, 255.0f);
